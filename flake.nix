{
  ...
  outputs = { self, nixpkgs, openclaw, nixos-cosmic, nix-index-database, ... }@inputs: {
    nixosConfigurations.charlie = nixpkgs.lib.nixosSystem {
      ...
      modules = [
        # wechat-uos 4.1.1 overlay（官方新版路径改为 /opt/wechat/）
        { nixpkgs.overlays = [
            (final: prev:
              let
              ...
              in
              {
                wechat-uos = prev.buildFHSEnv {
                  ...
                  '';
                  targetPkgs = pkgs: [
                    prev.util-linux   # lsblk, mount etc.
                    prev.coreutils
        ...
        ./configuration.nix
        {
          nix.settings = {
            ...
            trusted-public-keys = [ "cosmic.cachix.org-1:Dya9IyXD4HwqkHRG02q1PEnwEKfPByjV+G3eTEfkwEo=" ];
          };
        }
